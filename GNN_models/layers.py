...
def fully_con_layer(x, n, channel, scope,out_channel):
    '''
    ...
    '''
    w = tf.get_variable(name=f'w_{scope}', shape=[1, 1, channel, out_channel], dtype=tf.float32)
    tf.add_to_collection(name='weight_decay', value=tf.nn.l2_loss(w))
    b = tf.get_variable(name=f'b_{scope}', initializer=tf.zeros([n, out_channel]), dtype=tf.float32)
    return tf.nn.conv2d(x, w, strides=[1, 1, 1, 1], padding='SAME') + b


def output_layer(x, T, scope, out_channel, act_func='GLU'):
    '''
    ...
    #return x_o
    x_fc = fully_con_layer(x_o, n, channel, scope,out_channel)
    
    #x_o: Tensor("output_layer_out/Sigmoid:0", shape=(?, 1, 16, 128), dtype=float32) 
    #channel: 128 
    #x_fc: Tensor("add:0", shape=(?, 1, 16, 1), dtype=float32)
    return x_fc
