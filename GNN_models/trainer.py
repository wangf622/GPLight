...
def model_train(inputs, blocks, args, sum_path='./output/tensorboard'):
    ...
    x = tf.placeholder(tf.float32, [None, n_his + 1, n, args.channel], name='data_input')
    ...
    #pred: Tensor("strided_slice_4:0", shape=(?, 228, 1), dtype=float32)

    tf.summary.scalar('train_loss', train_loss)
    copy_loss = tf.add_n(tf.get_collection('copy_loss'))
    #copy_loss : Tensor("L2Loss_1:0", shape=(), dtype=float32)
    tf.summary.scalar('copy_loss', copy_loss)

    # Learning rate settings
    global_steps = tf.Variable(0, trainable=False)
    #global_steps : <tf.Variable 'Variable:0' shape=() dtype=int32_ref>
    
    len_train = inputs.get_len('train') #9112
    ...
    #lr: Tensor("ExponentialDecay:0", shape=(), dtype=float32)
    #step_op:  Tensor("AssignAdd:0", shape=(), dtype=int32_ref)
    ...
    with tf.Session() as sess:
        ...
        if inf_mode == 'sep':
            ...
            #[4.e+01, 1.e+05, 1.e+05, 4.e+01, 1.e+05, 1.e+05, 4.e+01, 1.e+05,1.e+05]
        else:
        ...
        for i in range(epoch): #50
            ...
            for j, x_batch in enumerate(
                ...
                if j % 50 == 0:
                    ...
                    with open(r'OUTPUT.txt','a') as record_file:
                        record_file.write(f'Epoch {i:2d}, Step {j:3d}: [{loss_value[0]:.3f}, {loss_value[1]:.3f}]\n')
            print(f'Epoch {i:2d} Training Time {time.time() - start_time:.3f}s')
            with open(r'OUTPUT.txt','a') as record_file:
                record_file.write(f'Epoch {i:2d} Training Time {time.time() - start_time:.3f}s\n')
            ...
            with open(r'OUTPUT.txt','a') as record_file:
                record_file.write(f'Epoch {i:2d} Inference Time {time.time() - start_time:.3f}s\n')
